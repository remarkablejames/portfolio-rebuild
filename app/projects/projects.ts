export const AllProjects = [
  {
    id: 1,
    name: "SomoLabs",
    time: "December 2023 - Present",
    description:
        "This project comprises a mobile app, a client-facing web application, and a content management system (CMS) for  a small SaaS startup, SomoLabs that I co-founded to revolutionize online academic education. SomoLabs is an innovative online learning platform designed to provide an engaging and structured approach to academic learning. " +
        "\n" +
        "As the front-end engineer, my role focuses on front-end design, performance optimization, and seamless integration with back-end services and third-party APIs, ensuring a smooth and responsive user experience across all platforms.",
    link: {
      href: "https://somolabs.com/",
      label: "Live preview",
      active: true,
    },
    images: [{ src: "https://res.cloudinary.com/dhka6cqkn/image/upload/f_auto,q_auto/v1/potfolio-images/project-somolabs/wbzyxivtd0ken67o0jw2", aspect_ratio: 16 / 9 },
      { src: "https://res.cloudinary.com/dhka6cqkn/image/upload/f_auto,q_auto/v1/potfolio-images/project-somolabs/ozjqeezdhuyzutkyo900", aspect_ratio: 16 / 9 },
      {
        src: "https://res.cloudinary.com/dhka6cqkn/image/upload/f_auto,q_auto/v1/potfolio-images/project-somolabs/dx6ve7ppn81q4uxp7efa",
        aspect_ratio: 16 / 9,
      },
      { src: "https://res.cloudinary.com/dhka6cqkn/image/upload/f_auto,q_auto/v1/potfolio-images/project-somolabs/w3clvqhssluigbbbucus", aspect_ratio: 16 / 9 },],
  },
  {
    id: 2,
    name: "Banquetly",
    time: "July 2023 - August 2023",
    description:
        "A minimalistic React Native app designed to streamline the management of hospitality event staff and their shift schedules. Built with React Native, TypeScript, and Firebase, this app empowers event organizers to efficiently manage staff assignments and schedules in a user-friendly interface.",
    link: {
      href: "https://github.com/remarkablejames/Banquetly",
      label: "Source code(GitHub)",
      active: true,
    },
  },

  {
    id: 3,
    name: "Distributed Task Queue System",
    time: "November 2024 - Present",
    description:
        " This is a system built with Go which enables tasks (or jobs) to be processed asynchronously across multiple worker nodes in a distributed environment. It decouples task creation from task execution, allowing scalability, fault tolerance, and efficient resource utilization.",
    link: {
      href: "https://github.com/remarkablejames/Banquetly",
      label: "Repo is private",
      active: false,
    },
  }
];

// export const AllProjects = [
//   {
//     id: 1,
//     name: "SomoLabs Client",
//     time: "December 2023 - Present",
//     description:
//       "This is a propeietary, production-grade online learning platform that offers curriculum-aligned courses delivered in multimedia format. My work on this project as a frontend engineer, primarily focuses on front-end design, performance optimization and its integration with the back-end and third-party APIs. This application is built with Next.js, TypeScript, and Tailwind CSS. I also work on the backend with Python and other technologies.",
//     link: {
//       href: "https://somolabs.com/",
//       label: "Live preview",
//       active: true,
//     },
//     images: [{ src: "https://res.cloudinary.com/dhka6cqkn/image/upload/f_auto,q_auto/v1/potfolio-images/project-somolabs/wbzyxivtd0ken67o0jw2", aspect_ratio: 16 / 9 },
//       { src: "https://res.cloudinary.com/dhka6cqkn/image/upload/f_auto,q_auto/v1/potfolio-images/project-somolabs/ozjqeezdhuyzutkyo900", aspect_ratio: 16 / 9 },
//       {
//         src: "https://res.cloudinary.com/dhka6cqkn/image/upload/f_auto,q_auto/v1/potfolio-images/project-somolabs/dx6ve7ppn81q4uxp7efa",
//         aspect_ratio: 16 / 9,
//       },
//       { src: "https://res.cloudinary.com/dhka6cqkn/image/upload/f_auto,q_auto/v1/potfolio-images/project-somolabs/w3clvqhssluigbbbucus", aspect_ratio: 16 / 9 },],
//   },
//   {
//     id: 2,
//     name: "Markdown compiler",
//     time: "May 2023 - October 2023",
//     description:
//       "This is a markdown compiler built with React and TypeScript. It takes markdown content as input and transpiles it to HTML. There are many markdown compilers out there, but this one is mine.",
//     link: {
//       href: "https://github.com/remarkablejames/markdown-transpiller",
//       label: "Open on GitHub",
//       active: true,
//     },
//   },
//   {
//     id: 3,
//     name: "SomoLabs Mobile",
//     time: "July 2023 - September 2023",
//     description:
//       "A mobile app that integrates with the SomoLabs online learning platform. This app is built with React Native and TypeScript.",
//     link: {
//       href: "https://github.com/brainwaveschool/SomoLabs-mobile",
//       label: "Source code is private",
//       active: false,
//     },
//   },
//   {
//     id: 4,
//     name: "SomoLabs CMS",
//     time: "January 2024 - Present",
//     description:
//       "A content management system for SomoLabs built with Next.js and Python Django",
//     link: {
//       href: "https://brainwave-admin.vercel.app/",
//       label: "Live preview",
//       active: true,
//     },
//     images: [{ src: "https://res.cloudinary.com/dhka6cqkn/image/upload/f_auto,q_auto/v1/potfolio-images/somolabs-cms/rtzfioywxq8ozdljr4jf", aspect_ratio: 16 / 9 },
//       { src: "https://res.cloudinary.com/dhka6cqkn/image/upload/f_auto,q_auto/v1/potfolio-images/somolabs-cms/uvga0wy9lgfivejklkcl", aspect_ratio: 16 / 9 },
//       { src: "https://res.cloudinary.com/dhka6cqkn/image/upload/f_auto,q_auto/v1/potfolio-images/somolabs-cms/jkhggtxryplcqodnem5f", aspect_ratio: 16 / 9 },
//       { src: "https://res.cloudinary.com/dhka6cqkn/image/upload/f_auto,q_auto/v1/potfolio-images/somolabs-cms/h9u8cgb5d0rupepmqj8h", aspect_ratio: 16 / 9 }],
//
//   },
//   {
//     id: 5,
//     name: "Netflix Clone",
//     time: "November 2023 - December 2023",
//     description: "Netflix Clone built with React, Redux, and Firebase",
//     link: {
//       href: "https://movies-app-james-one-green.vercel.app",
//       label: "Live preview",
//       active: true,
//     },
//   },
//   {
//     id: 6,
//     name: "Banquetly",
//     time: "July 2023 - August 2023",
//     description:
//       "Minimal React Native app for managing hospitality event staff and their shift schedules",
//     link: {
//       href: "https://github.com/remarkablejames/Banquetly",
//       label: "Source code(GitHub)",
//       active: true,
//     },
//   },
//
// //     ===========
//   {
//     id: 7,
//     name: "SomoLabs",
//     time: "December 2023 - Present",
//     description:
//         "I co-founded a small SaaS startup, SomoLabs, as a technical co-founder to revolutionize online academic education. SomoLabs is an innovative online learning platform designed to provide an engaging and structured approach to academic learning. The project comprises a mobile app, a client-facing web application, and a content management system (CMS).\n" +
//         "\n" +
//         "As the front-end engineer, my role focuses on front-end design, performance optimization, and seamless integration with back-end services and third-party APIs, ensuring a smooth and responsive user experience across all platforms.",
//     link: {
//       href: "https://somolabs.com/",
//       label: "Live preview",
//       active: true,
//     },
//     images: [{ src: "https://res.cloudinary.com/dhka6cqkn/image/upload/f_auto,q_auto/v1/potfolio-images/project-somolabs/wbzyxivtd0ken67o0jw2", aspect_ratio: 16 / 9 },
//       { src: "https://res.cloudinary.com/dhka6cqkn/image/upload/f_auto,q_auto/v1/potfolio-images/project-somolabs/ozjqeezdhuyzutkyo900", aspect_ratio: 16 / 9 },
//       {
//         src: "https://res.cloudinary.com/dhka6cqkn/image/upload/f_auto,q_auto/v1/potfolio-images/project-somolabs/dx6ve7ppn81q4uxp7efa",
//         aspect_ratio: 16 / 9,
//       },
//       { src: "https://res.cloudinary.com/dhka6cqkn/image/upload/f_auto,q_auto/v1/potfolio-images/project-somolabs/w3clvqhssluigbbbucus", aspect_ratio: 16 / 9 },],
//   }
// ];
